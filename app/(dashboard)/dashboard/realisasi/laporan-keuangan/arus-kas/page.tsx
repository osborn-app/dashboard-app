...
import { useGetCashFlow, GetCashFlowParams } from '@/hooks/api/useFinancialReports';
...
export default function ArusKasPage() {
  ...
  const params: GetCashFlowParams = {
  ...
  const { data: cashFlowData, isLoading, error, refetch } = useGetCashFlow(params);
  ...
  const handleExport = async (format: 'excel' | 'csv') => {
    ...
    try {
      ...
      if (cashFlowData.operating_activities?.length > 0) {
        cashFlowData.operating_activities.forEach((item: any) => {
          exportData.push({
            'No Akun': item.account_code || '-',
            'Nama Akun': item.account_name || '-',
            'Arus Kas Operasi': item.running_balance || 0,
      ...
      if (cashFlowData.investing_activities?.length > 0) {
        cashFlowData.investing_activities.forEach((item: any) => {
          exportData.push({
            'No Akun': item.account_code || '-',
            'Nama Akun': item.account_name || '-',
            'Arus Kas Operasi': 0,
            'Arus Kas Investasi': item.running_balance || 0,
            'Arus Kas Pendanaan': 0
          });
        });
      }
      ...
      if (cashFlowData.financing_activities?.length > 0) {
        cashFlowData.financing_activities.forEach((item: any) => {
          exportData.push({
            'No Akun': item.account_code || '-',
            'Nama Akun': item.account_name || '-',
            'Arus Kas Operasi': 0,
            'Arus Kas Investasi': 0,
            'Arus Kas Pendanaan': item.running_balance || 0
          });
        });
      }
      ...
      exportData.push({
        ...
        'Arus Kas Operasi': cashFlowData.net_cash_flow || 0,
        'Arus Kas Investasi': '',
        'Arus Kas Pendanaan': ''
      });
      ...
      if (format === 'csv') {
        ...
        downloadFile(csvContent, `arus_kas_${dayjs().format('YYYY-MM-DD')}.csv`, 'text/csv');
      } else {
        const excelContent = convertToExcel(exportData);
        downloadFile(excelContent, `arus_kas_${dayjs().format('YYYY-MM-DD')}.xlsx`, 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet', true);
      }
  ...
  const filteredOperatingActivities = filterAccounts(cashFlowData?.operating_activities || []);
  const filteredInvestingActivities = filterAccounts(cashFlowData?.investing_activities || []);
  const filteredFinancingActivities = filterAccounts(cashFlowData?.financing_activities || []);
    ...
    <>
      <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
        ...
        <div className="flex items-start justify-between">
          <Heading title="Laporan Arus Kas" description="Laporan arus kas realisasi keuangan" />
        </div>
        <Separator />
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle>Arus Kas Realisasi</CardTitle>
                      ...
                      <>
                          ...
                          filteredOperatingActivities.map((item: any, index: number) => (
                            <tr key={`operating-${index}`} className="bg-white hover:bg-gray-50 border-b border-gray-200">
                              ...
                              <td className="py-2 px-4 text-sm text-gray-700 text-right border-r border-gray-300">
                                {formatRupiah(item.running_balance || 0)}
                              </td>
                              <td className="py-2 px-4 text-sm text-gray-700 text-right border-r border-gray-300">
                                -
                              </td>
                        ...
                        <tr className="bg-blue-50 border-t-2 border-blue-400">
                          ...
                          <td className="py-3 px-4 text-sm font-bold text-blue-900 text-right border-r border-blue-300">
                            {formatRupiah(cashFlowData.net_cash_flow || 0)}
                          </td>
                          <td className="py-3 px-4 text-sm font-bold text-blue-900 text-right border-r border-blue-300">
                            -
                          </td>
