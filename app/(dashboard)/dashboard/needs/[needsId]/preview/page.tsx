            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-lg">{data?.data?.fleet?.name}</p>
                  </div>
                  <div>
                    <h3 className="text-sm font-medium text-gray-500">Plat Nomor</h3>
                    <p className="text-lg">{data?.data?.fleet?.plate_number}</p>
                  </div>
                  <div>
                    <h3 className="text-sm font-medium text-gray-500">Tanggal Mulai</h3>
                    <p className="text-lg">{new Date(data?.data?.start_date).toLocaleDateString('id-ID')}</p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-lg">{data?.data?.estimate_days} hari</p>
                  </div>
                  <div>
                    <h3 className="text-sm font-medium text-gray-500">Tanggal Dibuat</h3>
                    <p className="text-lg">{new Date(data?.data?.created_at).toLocaleDateString('id-ID')}</p>
                  </div>
                  <div>
                    <h3 className="text-sm font-medium text-gray-500">Terakhir Diupdate</h3>
                    <p className="text-lg">{new Date(data?.data?.updated_at).toLocaleDateString('id-ID')}</p>
                  </div>
                </div>
              </div>
              ...
              {data?.data?.description && (
                <div className="pt-4 border-t">
                  ...
                  <p className="text-gray-900 whitespace-pre-wrap">{data?.data?.description}</p>
                </div>
              )}
            </CardContent>
          </Card>
